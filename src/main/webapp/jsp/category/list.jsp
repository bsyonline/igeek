...
<div class="formSubmit">
  ...
  <input type="button" value="<spring:message code="button.category.add" text="Add Category"/>" class="redB" onclick="add(2)"/>
</div>
<!-- Dynamic table -->
<div class="widget">
  <div class="title">
      <img src="${pageContext.request.contextPath}/images/icons/dark/full2.png" alt="" class="titleIcon" />
      <h6><spring:message code="label.category.list" text="Category List"/> </h6>
  </div>
  <table cellpadding="0" cellspacing="0" border="0" class="display dTable">
    <thead>
    <tr>
<%--suppress CheckTagEmptyBody --%>
      <th><spring:message code="label.category.name" text="Category Name"/></th>
      <th>option</th>
    </tr>
    </thead>
    <tbody>
    ...
    <tr class="gradeA">
      <td><a href="javascript:doView('${c.id}')" >${c.name}</a></td>
      <td align="center">
...
<script>
  function doView(arg){
    $("#content-wrapper").empty().load("${pageContext.request.contextPath}/category/view/"+arg);
  }
  function doUpdate(arg){
    $("#content-wrapper").empty().load("${pageContext.request.contextPath}/category/update/"+arg);
  }
  function doDelete(arg){
    $("#content-wrapper").empty().load("${pageContext.request.contextPath}/category/delete/"+arg);
  }
</script>